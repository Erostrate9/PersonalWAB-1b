...
from utils import load_param_prompt, load_function_prompt, normalize_tool_prediction
import json
            ...
            with distributed_state.split_between_processes(tokenized_prompts) as batched_prompts:
                for batch in batched_prompts:
                    ...
                    if args.test_on == 'function':
                        beams = [
                            normalize_tool_prediction(
                                tokenizer.decode(x, skip_special_tokens=True).strip().split('### Tool:\n')[-1]
                            )
                            for x in beams
                        ]
